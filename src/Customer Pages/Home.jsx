...
// // Deduplicate by product name — home hero shows one slide per flavour (40g default)
// function buildSlides(variants) {
//   const seen = new Set();
//   return variants
//     .filter(v => {
...
//     .map(v => ({
//       bg:         v.bg,
//       burstLight: v.burstLight,
//       burstDark:  v.burstDark,
//       label:      v.name.toUpperCase(),
//       sub:        v.tagline.toUpperCase(),
//       btnBg:      "#FFD700",
//       imgSrc:     v.imgSrc,
//     }));
// }
...
//   // Loading skeleton — same colours, no layout shift
...
//             exit={{ opacity: 0, scale: 1.1 }}
//             transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
//             style={{ rotate: burstRotate, scale: burstScrollScale,
...
//               style={{ height: "clamp(410px, 50vh, 550px)", width: "auto",
//                 objectFit: "contain", filter: "drop-shadow(0 24px 40px rgba(0,0,0,0.4))" }}
//             />
//             <div className="text-center mt-1">
//               <p className="font-black uppercase text-white"
//                 style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(22px, 4vw, 40px)",
//                   letterSpacing: 1, textShadow: "0 2px 12px rgba(0,0,0,0.3)" }}>
//                 {slide.label}
//               </p>
//               <p className="text-white/70 text-sm tracking-[3px] mt-1 uppercase"
//                 style={{ fontFamily: "var(--font-body)" }}>
//                 {slide.sub}
//               </p>
//             </div>
...
//           transition={{ duration: 0.7 }}
//           className="absolute bottom-20 left-6 z-10 font-black leading-none pointer-events-none select-none"
//           style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(44px, 10vw, 110px)",
...
//             fontSize: 28, fontWeight: 900, transition: "background 0.2s", lineHeight: 1 }}>
//           {dir === -1 ? "‹" : "›"}
//         </motion.button>
//       ))}
...
export default function Home() {
  ...
  // FIX 1: Use a ref to always hold latest slides.length
  // so the interval callback never goes stale
  ...
  // FIX 2: stable changeSlide using ref — never goes stale inside interval
  ...
  // FIX 3: Fetch with retry for Render cold starts (can take 20–30s)
  useEffect(() => {
    let cancelled = false;
    let retryTimer = null;
    ...
    const fetchProducts = async (attempt = 1) => {
      try {
        ...
        if (cancelled) return;
        console.warn(`[Home] fetch attempt ${attempt} failed:`, err.message);
        // Retry up to 5 times with increasing delay (handles Render cold start)
        if (attempt < 5) {
          const delay = attempt * 4000; // 4s, 8s, 12s, 16s
          retryTimer = setTimeout(() => fetchProducts(attempt + 1), delay);
        } else {
          setLoading(false); // give up after ~40s
        }
      }
    };
  ...
  // FIX 4: Auto-slide — starts ONLY after slides are loaded, resets on manual nav
  // Uses functional setCurrent so it never closes over stale `current`
  useEffect(() => {
    if (slides.length <= 1) return; // no point sliding if 0 or 1 slide
    ...
    return () => clearInterval(t);
  }, [slides.length, current]); // reset timer when user manually changes slide
  ...
  if (loading || slides.length === 0) {
    return (
      ...
      >
        ...
        />
        {/* Show loading text after 5s so user knows it's a cold start */}
        <LoadingHint />
      </section>
    );
  }
    ...
    >
      ...
      {[-1, 1].map((dir) => (
        <motion.button
          ...
          className="absolute top-1/2 -translate-y-1/2 z-30 flex items-center justify-center border-none cursor-pointer shadow-2xl"
          style={{
            [dir === -1 ? "left" : "right"]: 18, width: 52, height: 52,
            borderRadius: "50%", background: "rgba(0,0,0,0.72)", color: "#fff",
            fontSize: 28, fontWeight: 900, transition: "background 0.2s", lineHeight: 1,
          }}
        >
...
// Shows a hint after 5s so the user knows the server is waking up
