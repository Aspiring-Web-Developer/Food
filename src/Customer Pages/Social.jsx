...
function VideoThumb({ video, onClick }) {
  return (
    ...
    >
      ...
      {/* Dark gradient overlay — deepens on hover, like Netflix/IG tiles */}
      <motion.div
        variants={{
          rest: { opacity: 0.15 },
          hover: { opacity: 0.55 },
      ...
      >
        <motion.div
          variants={{
            rest: { scale: 1, boxShadow: "0 4px 20px rgba(0,0,0,0.25)" },
            hover: {
              scale: 1.15,
        ...
        >
          <Play
            ...
            fill="#fff"
            color="#fff"
            style={{ marginLeft: 3 }} // optical centering — play icons look off-center otherwise
          />
        </motion.div>
      </div>

      {/* Subtle corner accent — brand touch, feels intentional not generic */}
      <div
        style={{
          position: "absolute",
          top: 12,
          ...
          borderRadius: "50%",
          background: "#E1306C",
          boxShadow: "0 0 8px rgba(225,48,108,0.8)",
        }}
      />
    </motion.div>
  );
...
useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="ig-grid">
  {videos.map((v, i) => (
    <VideoThumb key={v.id} video={v} onClick={() => setActiveIndex(i)} />
  ))}
</div>
...
<AnimatePresence>
  {activeIndex !== null && (
    <VideoModal video={videos[activeIndex]} onClose={() => setActiveIndex(null)} />
  )}
</AnimatePresence>
    </section>
  );
